...
export default function Listpage(){
    ...
    const[items, setItems] = useState([]); //initalize items as empty array
    ...
    const loadData = async () => {
        ...
        console.log("Length before the load is", items.length)
    ...
    return (
        <div>
            <h1>Welcome to the list page!</h1>
             ...
             { items.length < 1 ? <p>Length before is: {items.length}</p> :  <p>Length after load is: {items.length}</p>}
